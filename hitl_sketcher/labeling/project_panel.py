...
class ProjectPanel(QDockWidget):
    """Unified dock widget: project, classes, regions, SAM3 controls, and training inference."""
    ...
    def __init__(self, iface, client, viewer):
        super().__init__("Labelling", iface.mainWindow())
        self.iface = iface
        self.client = client
        self.viewer = viewer
    ...
    def _on_register_source(self) -> None:
        ...
        if not url:
            ...
            return
        name, ok = QInputDialog.getText(self, "Register Source", "Source name:")
        if not ok or not name.strip():
            return
        try:
    ...
    def _refresh_models(self) -> None:
        ...
        try:
            ...
            self._model_info.setText(f"{count} model{'s' if count != 1 else ''} available")
        except Exception:
    ...
    def _poll_status(self) -> None:
        try:
            ...
            if state == "complete":
                ...
                self._poll_timer.stop()
                self._run_status.setText(f"Error: {status.get('error_message', 'unknown')}")
